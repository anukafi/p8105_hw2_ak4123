...
The total precipitation in 2017 was `r {sum(seventeen_data$total)}` inches for the data available. 

The median number of sports balls in a dumpster in 2016 was `r {median(sixteen_data$sports_balls)}` balls. 
...
mutate(excellent_verygood = sum(sum(excellent), sum(verygood)))
mean(sum(excellent)) %>%
count(select(BRFSS_data, excellent_verygood) %>% summarize(mean(excellent_verygood)))
```
